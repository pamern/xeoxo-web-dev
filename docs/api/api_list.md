...
*   **Resource**: Auth
    ...
    *   **Ghi chú**: Flow đăng ký bằng SĐT được xử lý qua server bằng email alias nội bộ để không phụ thuộc xác minh SMS hoặc phone auth provider của Supabase.

*   **Resource**: Auth
    *   **Action**: Đăng nhập bằng Email/SĐT và mật khẩu
    *   **HTTP method**: `POST`
    *   **Endpoint**: `/api/v1/auth/signin`
    *   **Ghi chú**: Đăng nhập bằng SĐT được resolve qua email alias nội bộ trên server, không gọi trực tiếp phone auth ở client.
