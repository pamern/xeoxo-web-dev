...
Chưa có policy/grant frontend trực tiếp cho:

- `iam.loyalty_tier`
  - trạng thái hiện tại: chưa expose cho `authenticated` trong migration web access
  - hệ quả: UI membership không nên giả định có thể `SELECT` trực tiếp qua Supabase client cho tới khi có migration bổ sung
...
- `customization.customization_request`
  - `customization_request_self_select`
  - chỉ cho `SELECT`
  - điều kiện: `customer_id = util.current_customer_id()`
...
- `customization.measurement_appointment`
  - trạng thái hiện tại: chưa expose trong migration web access
  - hệ quả: các flow lịch hẹn hiện phải đi qua backend/service role hoặc cần migration bổ sung nếu muốn frontend gọi trực tiếp

Ghi chú triển khai:

- `customization.customization_request` hiện mới mở `SELECT` owner-based, chưa có `INSERT/UPDATE/DELETE` trực tiếp cho `authenticated`.
- Với các endpoint backend dùng `service_role` để đọc `customization_request`, vẫn phải tự kiểm tra ownership theo `customer_id`; RLS không bảo vệ được khi service role bypass policy.
- `sales.cart_item.customization_snapshot` và `sales.order_item.customization_snapshot` là snapshot giao dịch, không được suy diễn ngược từ `measurement_profile` ở thời điểm đọc dữ liệu cũ.
