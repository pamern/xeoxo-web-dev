        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <button
            ...
            className={cn(
              ...
              isDefault
                ? "cursor-not-allowed border-black/20 bg-black/[0.02] text-foreground/35"
...
export default async function AccountAddressesRoute() {
  ...
  const addresses =
    customer?.customer_id != null
      ? await getCustomerAddressesByCustomerId(customer.customer_id)
      : [];
                ...
                {isAuthenticated ? (
                    ...
                    {addresses.length ? (
                      ...
                      <div className="rounded-[20px] border border-black/12 bg-secondary px-6 py-10">
                        ...
                        <p className="mt-3 max-w-[620px] text-sm font-light leading-relaxed text-foreground/72 md:text-base">
                          Khi API địa chỉ được chốt theo docs, phần thêm và cập nhật địa
                          chỉ sẽ được nối vào ngay tại màn hình này.
                        </p>
                      </div>
                    )}
                  ...
                  <div className="mt-8 rounded-[20px] border border-border bg-secondary px-6 py-8">
                    <p className="text-lg font-medium">
                      Bạn cần đăng nhập để xem sổ địa chỉ.
                    </p>
                    <p className="mt-2 text-sm font-light text-foreground/72">
                      Sau khi đăng nhập, trang này sẽ hiển thị các địa chỉ giao hàng gắn
                      với tài khoản của bạn trong hệ thống.
                    </p>
                    <div className="mt-6">
                      <Link
